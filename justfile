...
[group('Development')]
preview:
	binary_dir="$({{ uv }} scripts/swift_build.py --configuration debug --show-bin-path)"; executable_name="$({{ uv }} scripts/project_config.py executable-name)"; "$binary_dir/$executable_name" --developer-preview
