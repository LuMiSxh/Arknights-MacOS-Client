...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------- |
| Regional game files         | `~/Library/Application Support/com.lumisxh.arknights-client/{Yostar/{Global,Japan,Korea},Gryphline/Taiwan,Hypergryph/{China,China-Bilibili}}` | `InstallationController` / `GameInstaller` | Updated by the manifest; **Uninstall Game** moves only the selected directory to the Trash                                       |
...
| Wine prefixes               | `~/Library/Application Support/com.lumisxh.arknights-client/{Yostar,Gryphline,Hypergryph}/Prefix`                                             | `GameSessionController` / `WineRuntime`    | Shared within the publisher family; deleting one reruns its migration plan                                                       |
...
| Launcher and runtime logs   | `~/Library/Logs/com.lumisxh.arknights-client`                                                                                                 | `LauncherLog` and runtime process output   | `launcher.log`, publisher logs including `arknights-gryphline.log`, `unity.log`, and `chromium.log`; **Show Logs** prepares them |
...
2. The installer verifies its size and provider checksum (CRC64 or MD5), then moves it into the final destination.
