# SPDX-License-Identifier: MPL-2.0

"""GitHub issue and retained-report adapter for runtime monitoring."""

from __future__ import annotations

import io
import json
import re
import urllib.parse
import zipfile

from lib.common import ScriptError, fail
from lib.github_client import GitHubClient
from lib.runtime_monitor import (
    AUTOMATED_LABEL,
    RuntimeMonitorIssue,
    RuntimeMonitorReport,
)

REPORT_ARTIFACT_NAME = "runtime-monitor-report"
MAXIMUM_REPORT_ARCHIVE_BYTES = 2 * 1_024 * 1_024
MAXIMUM_REPORT_BYTES = 1_024 * 1_024
REPOSITORY_PATTERN = re.compile(r"^[A-Za-z0-9_.-]+/[A-Za-z0-9_.-]+$")


class RuntimeGitHubIssueStore:
    def __init__(self, client: GitHubClient, repository: str) -> None:
        if REPOSITORY_PATTERN.fullmatch(repository) is None:
            fail("GITHUB_REPOSITORY has an invalid value")
        self.client = client
        self.repository = repository

    def ensure_automated_label(self) -> None:
        try:
            self.client.repository_json(
                self.repository,
                "/labels",
                method="POST",
                payload={
                    "name": AUTOMATED_LABEL,
                    "color": "6E7781",
                    "description": "Created and maintained by repository automation",
                },
            )
        except ScriptError as error:
            if "status 422" not in str(error):
                raise

    def list_automated_issues(self) -> list[RuntimeMonitorIssue]:
        query = urllib.parse.urlencode(
            {"state": "all", "labels": AUTOMATED_LABEL, "per_page": 100}
        )
        value = self.client.repository_json(self.repository, f"/issues?{query}")
        if not isinstance(value, list):
            fail("GitHub returned an invalid automated issue list")
        return [
            RuntimeMonitorIssue(
                number=int(item["number"]),
                state=str(item["state"]),
                body=str(item.get("body") or ""),
            )
            for item in value
            if isinstance(item, dict) and "pull_request" not in item
        ]

    def create_issue(self, title: str, body: str) -> RuntimeMonitorIssue:
        value = self.client.repository_json(
            self.repository,
            "/issues",
            method="POST",
            payload={"title": title, "body": body, "labels": [AUTOMATED_LABEL]},
        )
        if not isinstance(value, dict):
            fail("GitHub returned invalid created issue metadata")
        return RuntimeMonitorIssue(
            number=int(value["number"]),
            state=str(value["state"]),
            body=str(value.get("body") or ""),
        )

    def update_issue(
        self, number: int, *, body: str | None = None, state: str | None = None
    ) -> None:
        payload: dict[str, str] = {}
        if body is not None:
            payload["body"] = body
        if state is not None:
            payload["state"] = state
            if state == "closed":
                payload["state_reason"] = "completed"
        self.client.repository_json(
            self.repository, f"/issues/{number}", method="PATCH", payload=payload
        )

    def add_comment(self, number: int, body: str) -> None:
        self.client.repository_json(
            self.repository,
            f"/issues/{number}/comments",
            method="POST",
            payload={"body": body},
        )


def historical_reports(
    client: GitHubClient, repository: str, current_run_id: str, limit: int = 4
) -> list[RuntimeMonitorReport]:
    query = urllib.parse.urlencode(
        {"name": REPORT_ARTIFACT_NAME, "per_page": min(limit + 4, 100)}
    )
    value = client.repository_json(repository, f"/actions/artifacts?{query}")
    artifacts = value.get("artifacts", []) if isinstance(value, dict) else []
    reports: list[RuntimeMonitorReport] = []
    for artifact in artifacts:
        if not isinstance(artifact, dict) or artifact.get("expired") is True:
            continue
        workflow_run = artifact.get("workflow_run")
        run_id = str(workflow_run.get("id")) if isinstance(workflow_run, dict) else ""
        if run_id == current_run_id:
            continue
        try:
            archive = client.artifact_archive(
                repository, int(artifact["id"]), MAXIMUM_REPORT_ARCHIVE_BYTES
            )
            with zipfile.ZipFile(io.BytesIO(archive)) as zipped:
                info = zipped.getinfo("runtime-monitor-report.json")
                if info.file_size > MAXIMUM_REPORT_BYTES:
                    raise ValueError("runtime monitor report is too large")
                report = RuntimeMonitorReport.from_json(json.loads(zipped.read(info)))
        except (
            KeyError,
            TypeError,
            ValueError,
            json.JSONDecodeError,
            zipfile.BadZipFile,
        ):
            continue
        if report.trigger == "schedule":
            reports.append(report)
    return sorted(reports, key=lambda item: item.checked_at, reverse=True)[:limit]
